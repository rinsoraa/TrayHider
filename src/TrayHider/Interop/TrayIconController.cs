...
internal static class TrayIconController
{
    ...
    public static bool HideIcon(IntPtr hWnd, uint uID)
    {
        try
        {
    ...
    /// </summary>
    public static bool ShowIcon(IntPtr hWnd, uint uID, byte[]? iconSnapshot, string tooltip)
    {
        try
        {
            var hIcon = LoadIconFromSnapshot(iconSnapshot);
            var nid = new NOTIFYICONDATAW
            {
                cbSize = (uint)Marshal.SizeOf<NOTIFYICONDATAW>(),
                hWnd = hWnd,
    ...
    /// </summary>
    public static IntPtr FindTrayWindow(string processName, uint uid, string tooltip)
    {
        ...
        {
            if (ProbeMatch(hwnd, uid, tooltip)) return hwnd;
        }
    ...
    /// <summary>用 NIM_MODIFY 探测某个窗口是否持有指定 uID 的托盘图标（无副作用）。</summary>
    private static bool ProbeMatch(IntPtr hwnd, uint uid, string tooltip)
    {
        try
        {
            var nid = new NOTIFYICONDATAW
    ...
    /// <summary>从注册表 IconSnapshot 字节重建图标句柄，失败回退到系统默认图标。</summary>
    private static IntPtr LoadIconFromSnapshot(byte[]? snapshot)
    {
        try
        {
