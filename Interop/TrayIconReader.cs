...
internal sealed class TrayIconReader
{
    ...
    /// <summary>读取全部注册过的托盘图标条目。默认不读取 IconSnapshot，避免高频大对象分配。</summary>
    public IReadOnlyList<TrayIconEntry> ReadAll(bool includeIconSnapshot = false)
    {
        ...
        try
        {
            ...
            foreach (var subKeyName in root.GetSubKeyNames())
            {
                ...
                var entry = new TrayIconEntry
                {
                    ...
                    IconSnapshot = includeIconSnapshot ? ReadIconSnapshot(key) : null
                };
    ...
    /// <summary>仅在首次隐藏、恢复或需要图标兜底时读取 IconSnapshot。</summary>
    public static byte[]? ReadIconSnapshot(string subKeyName)
    {
        try
        {
            using var key = Registry.CurrentUser.OpenSubKey($@"{RegistryPath}\{subKeyName}");
            return key == null ? null : ReadIconSnapshot(key);
        }
        catch
        {
            return null;
    ...
    private static byte[]? ReadIconSnapshot(RegistryKey key)
        => key.GetValue("IconSnapshot") as byte[];
